...
function App() {
  ...
  const postData = async (newThreads) => {
  ...
  return (
    <div className="appContainer">
      ...
      <ThreadInput threads={threads} setThreads={setThreads} username={username} postData={postData} />
      <div className="feed">
        {threads && threads.map((thread) => (
          <Thread key={thread.id} data={thread} setThreads={setThreads} postData={postData} />
        ))}
      </div>
    </div>
  );
